...
# summary model
simulation_summary_at <- read_rds("data/absolute_thresholds/simulation_summary_absolute.rds.bz2")

## transform data to obtain trial summaries
choice_data_summary_at <- simulation_summary_at %>%
  group_by(psi, threshold, theta, problem, agent) %>%
  mutate(n_sample = n(), # total number of single samples
         n_s = sum(is.na(r)), # number of single samples drawn from safe option
         n_r = n_sample - n_s, # number of single samples drawn from risky option
...
## tidy data 
choice_data_summary_at <- choice_data_summary_at %>% 
  mutate(psi = 1-(psi+.5), 
         model = "summary") %>% # to interpret psi as switching probability
  select(model, psi:problem, rare, p_r_low:ev_ratio, agent, n_sample, n_s, n_r, ep_r_low, ep_r_high, mean_r, r_sum, s_sum, diff, choice)

# roundwise model
simulation_roundwise_at <- read_rds("data/absolute_thresholds/simulation_roundwise_absolute.rds.bz2")

## transform data to obtain trial summaries
choice_data_roundwise_at <- simulation_roundwise_at %>% 
  group_by(psi, threshold, theta, problem, agent) %>% 
  mutate(n_sample = n(),
         n_s = sum(is.na(r)),
         n_r = n_sample - n_s, 
...
## tidy data
choice_data_roundwise_at <- choice_data_roundwise_at %>% 
  mutate(psi = 1-(psi+.5),
         model = "roundwise") %>%
  select(model, psi:problem, rare, p_r_low:ev_ratio, agent, n_sample, n_s, n_r, ep_r_low, ep_r_high, mean_r, r_sum, s_sum, diff, choice)

# join data sets and save as compressed data file
choice_data_at <- bind_rows(choice_data_summary_at, choice_data_roundwise_at)
checksum_choice_data_at <- digest(choice_data_at, "sha256")
write_rds(choice_data_at, "data/absolute_thresholds/choice_data_absolute.rds.bz2", compress = "bz2")

# full data set -----------------------------------------------------------

choice_data_merged <- bind_rows(choice_data, choice_data_at)
checksum_choice_data_merged <- digest(choice_data_merged, "sha256")
write_rds(choice_data_merged, "data/merged/choice_data_merged.rds.bz2", compress = "bz2")