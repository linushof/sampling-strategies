...
choice_data_summary <- simulation_summary %>%
  ...
  mutate(n_sample = n(), # total number of single samples
         n_r = n_sample - sum(is.na(r)), # number of single samples drawn from risky option
...
choice_data_summary <- choice_data_summary %>% 
  ...
  select(psi:problem, rare, p_r_low:ev_ratio, agent, n_sample, n_r, ep_r_low, ep_r_high, mean_r, r_sum, s_sum, diff, choice)
